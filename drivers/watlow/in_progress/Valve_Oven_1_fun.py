...
Subcode for Valve Oven Zone 1 parameters. This code is to be called from Main_temperature_control 
This code sets up the parameters for Valve Oven Zone 1 analog input, contorl loop, output, and alarms. 
...
def setup_valve_oven_1(instrument: minimalmodbus.Instrument, valve_oven_temp_setting: float) -> None:
    ...
    VO1_CL_h_Pb = instrument.write_register(4110, 1.00, 2)  # heat proportional band
    VO1_CL_h_hy = instrument.write_register(4120, 0.50, 2)  # heat hysteresis
    VO1_Cl_rP = instrument.write_register(5246, 88, 0)  # Ramp Action
    VO1_Cl_r_SC = instrument.write_register(5248, 57, 0)  # Rampe Scale
    VO1_Cl_r_rt = instrument.write_register(5252, 5.00, 2)  # Ramp Rate
    VO1_Cl_C_SP = instrument.write_register(5220, valve_oven_temp_setting, 2)  # set point
    VO1_Cl_id_S = instrument.write_register(5236, valve_oven_temp_setting, 2)  # idle set point
    ...
    VO1_A1_Sr_A = instrument.write_register(2692, 142, 0)  # Alamr Source
    ...
    VO1_A1_A_Low = instrument.write_register(2662, -1.00, 0)  # Low setpoint
    VO1_A1_A_hi = instrument.write_register(2660, 1.00, 0)  # High setpoint)
